...
// Convert IEEE 754 big-endian to float
...
// Read 2 input registers (32-bit float) from SDM120M
esp_err_t read_float_register(uint16_t reg_addr, float *result)
{
  ...
  vTaskDelay(pdMS_TO_TICKS(2)); // Increased

  uart_write_bytes(MODBUS_UART, request, 8);
  uart_wait_tx_done(MODBUS_UART, pdMS_TO_TICKS(100));

  vTaskDelay(pdMS_TO_TICKS(5)); // Increased

  // RX mode
  gpio_set_level(DE_RE_PIN, 0);
  vTaskDelay(pdMS_TO_TICKS(2)); // Wait for meter to respond
...
void read_task(void *arg)
{
  ...
  while (1)
  {
    ...
    if (read_float_register(0, &voltage) == ESP_OK)
    {
    ...
    if (read_float_register(6, &current) == ESP_OK)
    {
      ESP_LOGI(TAG, "Current: %.2f A", current);
    }

    if (read_float_register(12, &power) == ESP_OK)
    {
      ESP_LOGI(TAG, "Active Power: %.2f W", power);
    }

    if (read_float_register(26, &energy) == ESP_OK)
    {
...
void app_main(void)
{
  ...
  xTaskCreate(read_task, "sdm120m_read", 2048, NULL, 5, NULL);
}